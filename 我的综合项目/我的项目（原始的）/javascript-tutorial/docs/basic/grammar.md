...
上面的代码先声明变量`a`，然后在变量`a`与数值1之间建立引用关系，称为将数值1“赋值”给变量`a`。以后，引用变量名`a`就会得到数值1。最前面的`var`，是变量声明命令。它表示通知**解释引擎**，要创建一个变量`a`。
...
JavaScript 是一种动态类型语言，也就是说，变量的类型没有限制，变量可以随时更改类型。**（动态类型语言的解释，即，变量的类型是可以变化的。类型在运行中是可以改变的。）**
...
上面代码中，变量`x`声明了两次，第二次声明是无效的。**（第二次声明，是无效的，但是不会报错。仍然按照第一次的生命和赋值情况走。）**
