...
面向对象编程很重要的一个方面，就是对象的继承。A 对象通过继承 B 对象，就能直接拥有 B 对象的所有属性和方法。这对于  代码复用  是非常有用的。
...
通过构造函数为实例对象定义属性，虽然很方便，但是有一个缺点。同一个构造函数的多个实例之间，无法共享属性，从而造成对系统资源的浪费。 (多个实例共享属性和方法的需求。)
...
对于普通函数来说，该属性基本无用。但是，对于构造函数来说，生成实例的时候，该属性会自动成为实例对象的原型。 **每个实例对象都有原型。**
...
总结一下，原型对象的作用，就是定义所有实例对象共享的属性和方法。这也是它被称为 **原型对象** 的原因，**而实例对象可以视作从原型对象衍生出来的子对象。**
...
null instanceof Object // false  null 虽然是对象，但是不是Object的
```
