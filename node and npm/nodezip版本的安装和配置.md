...
解压文件到 指定目录 , 并在解压后的目录下建立 node_global和node_cache ，这两个文件夹的建立并不能自动实现 npm 的全局安装路径和全局缓存路径的设置。需要后面介绍的配置命令。
...
1. 安装 cnpm 命令。cnpm 安装完成后，以后就可以用cnpm代替 npm, 此时npm还是会用官方镜像，cnpm会用国内镜像
